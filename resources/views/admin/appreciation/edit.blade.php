...
@section('content')
    ...
    @endif
    <div class="container ">
        <form action="{{ route('appreciation.update', $appreciation->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6 mt-3">
                    ...
                    {{-- <input type="text" class="form-control" value="{{ $appreciation->title }}" id="title"
                        name="title" required> --}}
                    <select class="form-control" id="title" name="title" required>
                        <option value="{{ $appreciation->id }}">{{ $appreciation->title }}</option>
            ...
            <div class="form-row">
                <div class="form-group col-md-6 mt-3">
                    ...
                    <input type="date" class="form-control" id="given_date" value="{{ $appreciation->given_date }}"
                        name="given_date" required min="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
                </div>
            </div>
